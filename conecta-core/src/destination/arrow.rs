...
pub fn get_arrow_builders(schema: &Schema, capacity: usize) -> Vec<Box<dyn ArrayBuilder>> {
    ...
    for column in schema.columns.iter() {
        match column.data_type {
            NativeType::String => builders.push(Box::new(StringBuilder::with_capacity(capacity, 1024))),
            ...
            NativeType::VecString => builders.push(Box::new(ListBuilder::new(StringBuilder::new()))),
            NativeType::VecUUID => {
